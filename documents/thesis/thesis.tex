...
% List of simulations {{{ %
\newpage
\subsection{List of Simulation Classes}
\label{sec:simulation_list}
These are the predifined simulation classes included in LFPyutil. 
Each class has a simulation function that sets up 
electrodes and/or recording positions. 
Here the function and implementation 
...
%{{{ Blue brain models. 
\newpage
\section{Blue Brain Model Simulations}
...
Each of the models a me-type 
were implied to have slightly different morphological
shape and/or firing pattern. 
The degree of difference is not made clear in the documentation. 
...
To simulate this type of positioning of electrodes, the electrodes in the simulations
were placed at random location around the soma. 
...
of 
\SI{60}{\micro\metre}. 
This max distance were chosen because even the models with the strongest
...
The electrodes were placed and measured using the 
SphereRand 
(\cref{sec:simulation_list}) 
simulation class.
...
\begin{figure}[h]
    \begin{center}
        ...
        \caption{Spike width and amplitude histograms. 
        Right column shows the combined histogram of spike amplitude and 
        width; 
        left column shows the distribution of only spike width. 
        The overlap of the distributions are greater when 
        not considering the amplitude. 
        The overlap of the distributions for 
        spike width alone is $12.15\%$. 
        The overlap for the combined distribution of amplitude and width
        is $5.10\%$. 
        Data basen on 240 interneuron models and 20 pyramidal models. 
        }
...
Each of these classes are a me-type but also represents the
morphological class, the m-type, as the pyramidal neurons only have one 
e-type. 
The e-type of signifies a similar firing pattern based on features such as 
XXX. 
...
each of the classes.
The two groups TTPC1 and TTPC2 has a high overlap of XX\%
and as such they cannot be distingushed from another. 
The group UTPC has little overlap with both TTPC1 and TTPC2, but
overlaps STPC with XX\%. 


{\bf Interneurons:}
There is no clear multimodal distribution 
in the interneuron group but it might still be possible 
to assign models into subclasses. 
...
to remove noise
it is interesting to see if the same separation between interneurons
and pyramidal neurons hold when the data is filtered. 
...
the spike widths and and amplitude found in this project 
with spike data from other sources. 
The sources used where some chosen articles, 
data from the bio lab and online sources. 
