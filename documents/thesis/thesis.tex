...
% NEURON models all compartments as cylindrical geometries with varying
% diameters. 
...
Though 
only the capacitive current is proportional to the
the extracellular potential, 
(as can bee seen from \cref{eq:hux_current_eq}).
This is only the case in simple models. 
As the complexity of the morphology increases 
and the potential has increasingly many contributions 
the EAP can change. 
% For a two-compartmental mode, the smallest
% possible model that creates an extracellular potential, 
% the 
...
$R_m = \SI{30}{\kilo\ohm\per\centi\metre}$, 
...
\textcite{anastassiou_cell_2015}.
It is likely that the models were not created to
replicate these neuronal features. 
