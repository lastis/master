...
The main feature of the python package LFPy is to enable the calculation
of extracellular potentials, but another major feature is the simplification
of the NEURON simulation environment.
Using NEURON requires a good understanding of the programming language hoc
which creates many challenges for new users.
Hoc is not good.
While NEURON has a python interface
it gives the distinct feel of being a python to hoc interpreter

While NEURON has support for paralell processing it does not have 
any inherit support for multiple independent simulations at once.
% TODO: sources, docs
...
LFPyUtil uses pythons multiprocessing package to run independent simulations.

% In addition, troubleshooting programs Neuron is difficult because of the 
% relatively small user base compared 
% In addition to this the python interface to Neuron has been implemented 
% in such a way that it runs hoc code directly "under the hood". 
...
% This does not speed up a single simulation
...
These MWEs explain how to create a new custom simulation from
...
defines a custom simulation class that 
inherits the LFPyUtil simulation class\\
\verb+LFPyUtil.sims.Simulation+.
The new class has functions that inserts a stimulus electrode 
and applies a current to soma.
...
by calling the three functions as seen in listing ??.
%TODO: create the listing.

If one attempted to run the simulation function in 
listing ?? more than once in the same program,
we would experience that the latter simulations would be affected by the previous
simulations. 
This is because a new stimulation electrode will be applied 
once for each new simulation.
With Neuron and LFPy there are no easy way to reset the environment
to prevent this. 
One workaround is to use pythons multiprocessing package
to create multiple independent processes.
Doing this for every simulation will require a good amount of code,
to make this easier LFPyUtil has a class
\verb+LFPyUtil.Simulatior+
that can run the simulations in independent processes.

The class
\verb+LFPyUtil.Simulatior+
accepts one or more objects that inherit the 
\verb+LFPyUtil.Simulation+
class and can run them either in serial or paralell
and either in a new independent processes or in the same process. 
Listing ?? shows how the newly created simulation class can be used with
\verb+LFPyUtil.Simulatior+
that runs multiple simulations in parallel and in independt processes.

% \Cref{code:first_simulation} shows how to run a predefined simulation and plot 
% the results. 
% This simulation is not run in an independt process.
