...
def set_parameters():
    ...
    ### Na activation
    p['alpha_m'] = np.vectorize(lambda v: 0.38*(v+29.7)/
            (1.-np.exp(-(v+29.7)*0.1))) 
    ...
    ### K activation
    p['alpha_n'] = np.vectorize(lambda v: 0.02*(v+45.7)/
            (1.-np.exp(-0.1*(v+45.7))))
    p['beta_n'] = lambda v: 0.25*np.exp(-0.0125*(v+55.7))
    

    # Steven-connors addition
    p['a_inf'] = lambda v: (0.0761*np.exp(0.0314*(v+94.22))/
            (1+np.exp(0.0346*(v+1.17))))**(1/3.0)
...
def derived_parameters(p):
    ...
    p['time'] = np.arange(p['t_start'], p['T']+p['dt'], p['dt'])                          ## time array (ms)
...
if __name__ == "__main__":
    ...
    width, trace = LFPy_util.data_extraction.findWaveWidthsSimple(v_vec,0.5,dt=p['dt'])
    print 'Width = ', width
    print 'Amp = ', v_vec.max()
    ...
    LFPy_util.plot.fourierSpecter(freqs, amps,
            fname='fourier',
            f_end=3,
            show=False
    )
