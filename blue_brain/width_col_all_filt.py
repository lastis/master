...
widths_I = [x for (y,x) in sorted(zip(neuron_names, widths_I))]
widths_II = [x for (y,x) in sorted(zip(neuron_names, widths_II))]
amps_II = [x for (y,x) in sorted(zip(neuron_names, amps_II))]
...
size_amp = 20
size_width = 20
amp_width_hist = np.zeros([len(neuron_names)/5, size_amp-1,size_amp-1])
width_bin_edges = np.linspace(0, 2.5, size_width)
amp_bin_edges = np.linspace(0,300, size_amp)
...
for i in xrange(len(widths_I)):
    ...
    amp_width_hist[i/5] += hist

for i in xrange(amp_width_hist.shape[0]):
    amp_width_hist[i] /= amp_width_hist[i].sum()

jaccard_index = np.zeros([len(neuron_names)/5, len(neuron_names)/5])
for i in xrange(jaccard_index.shape[0]):
    for j in xrange(jaccard_index.shape[1]):
...
plt.colorbar()
plt.clim(0,1.0)
lplot.save_plt(plt, "hist_amp_width_meaned", dir_output)

plt.imshow(jaccard_index>0.2, interpolation="nearest")
lplot.save_plt(plt, "hist_amp_width_meaned_thresh", dir_output)
...
plt.imshow(jaccard_index>0.1, interpolation="nearest")
